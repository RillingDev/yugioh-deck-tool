...
function initComputed (vm, computed) {
  "developement" !== 'production' && checkOptionType(vm, 'computed');
...
function initMethods (vm, methods) {
  "developement" !== 'production' && checkOptionType(vm, 'methods');
  var props = vm.$options.props;
...
function initWatch (vm, watch) {
  "developement" !== 'production' && checkOptionType(vm, 'watch');
...
function resolveInject (inject, vm) {
  if (inject) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if ("developement" !== 'production' && !source) {
...
function on (el, dir) {
  if ("developement" !== 'production' && dir.modifiers) {
...
function genOnce (el, state) {
  ...
  if (el.if && !el.ifProcessed) {
    ...
    if (!key) {
      "developement" !== 'production' && state.warn(
...
const deckCardsWithoutPriceData = function () {
    ...
    eachObject$1(vm.deck.list, deckpart => {
