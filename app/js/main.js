...
var config = {
   ...
   */
  performance: "developement" !== 'production',
...
function mountComponent (
  ...
  var updateComponent;
  /* istanbul ignore if */
  if ("developement" !== 'production' && config.performance && perf) {
...
function leave (vnode, rm) {
  ...
  if ("developement" !== 'production' && explicitLeaveDuration != null) {
  ...
  parseHTML(template, {
    ...
    start: function start (tag, attrs, unary) {
      ...
      function checkRootConstraints (el) {
        if ("developement" !== 'production' && !warned) {
      ...
      if (!root) {
        ...
        } else if ("developement" !== 'production' && !warned) {
          warned = true;
          warn$2(
    ...
    chars: function chars (text) {
      if (!currentParent) {
        if ("developement" !== 'production' && !warned && text === template) {
          warned = true;
          warn$2(
...
function makeAttrsMap (attrs) {
  ...
  for (var i = 0, l = attrs.length; i < l; i++) {
    if ("developement" !== 'production' && map[attrs[i].name] && !isIE) {
