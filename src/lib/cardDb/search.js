    ...
    cardArr
      ...
      .filter(pair => {
        ...
        return (
          ...
          // Search Monster Sub
          (!is.monster || optionFilter(pairData.attribute, filter.attribute)) &&
          (!is.monster || optionFilter(pairData.race, filter.race)) &&
          (!is.monster ||
            optionFilter(String(pairData.stats[2]), filter.level)) &&
          (!is.monsterLink ||
            optionFilterArr(pairData.linkarrows, filter.linkarrows)) &&
          // Search Spell sub
          (!is.spell || optionFilter(pairData.race, filter.spelltype)) &&
          // Search Trap sub
